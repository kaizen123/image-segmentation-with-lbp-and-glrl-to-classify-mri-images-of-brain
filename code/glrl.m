function [SRE,LRE,GLN,RP,RLN,LGRE,HGRE,SRLGE,SRHGE,LRLGE,LRHGE]  = glrl(img,quantize,mask)
...
%     p0(i,:) = histogram(lengths0,1:maximgS);
%     p45(i,:) = histogram(lengths45,1:maximgS);
%     p90(i,:) = histogram(lengths90,1:maximgS);
%     p135(i,:) = histogram(lengths135,1:maximgS);    
end
...
% t = sum(p,1) ./ ((1:maximgS).^2);
% l = t ./ ((1:quantize)'.^2);
% g = size(p);
% SRLGE = sum(l,2)/ totSum;
k1 = 0;
for i = 1:quantize
    for j = 1:maximgS
        k1 = k1 + p(i,j)/(i^2+j^2);
